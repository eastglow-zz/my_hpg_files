...
interwidth = 0.28 #Target full-interfacial Width (in mm) - 7-internode-width for 2nd order elem
...
[GlobalParams]
  ...
  #gravity = '0 0 0'
  # gravity = '0 -9.81e-3 0'  #horizontal surfaces in 2D
  gravity = '0 0 -9.81e-3'  #horizontal surfaces in 3D
...
[Functions]
  ...
  [./TheCircle]
    ...
    vars = 'x0 y0 z0  r0'
    vals = '0  0  3.2   1.5'
    value = 'r:=sqrt((x-x0)^2+(y-y0)^2+(z-z0)^2);if(abs(r-r0) <= 0.5*${interwidth}, 0.5-0.5*sin(${pi}*(r-r0)/${interwidth}), if(r-r0 <= -0.5*${interwidth}, 1, 0))'
  ...
  [./FlatSlab]
    ...
    vars = 'z0'
    vals = '1'
    value = 'if(abs(z-z0) <= 0.5*${interwidth}, 0.5-0.5*sin(${pi}*(z-z0)/${interwidth}), if(z-z0 <= -0.5*${interwidth}, 1, 0))'
  [../]
  # [./TheSurface]
  ...
  [./sharklet_image_func]
    ...
    file_base = sharklet_images_stack/SharkletRef
    file_suffix = png
    file_range = '0 255'
    component = 0
    scale = 0.00392156862
  ...
  [./Background]
    ...
    functions = 'OneConst TheCircle sharklet_image_func'
    w =         '1        -1        -1'
  [../]
[]
...
[Kernels]
  ...
  [./cL_advection]
    ...
    vel_z = v_z
  [../]
  # [./cL_advection_constraint]
  #   type = ConstantMaterialAdvection
  #   variable = cL
  #   vel_x = v_x
  #   vel_y = v_y
  #   mat_name = mL
...
[Materials]
  ...
  [./Fractional_mobility_L]
    type = ParsedMaterial
    f_name = mL
    material_property_names = 'ML MV MS'
    function = 'ML/(ML+MV+MS)'
  [../]

  [./Fractional_mobility_V]
    type = ParsedMaterial
    f_name = mV
    material_property_names = 'ML MV MS'
    function = 'MV/(ML+MV+MS)'
  [../]
  ...
  [./kinetic_energy_density]
    ...
    function = '0.5 * rho * (v_x^2 + v_y^2 + v_z^2)'
  [../]
...
[Preconditioning]
  ...
  # Use with solve_type = PJFNK in Executioner block
  # petsc_options_iname = '-pc_type -pc_hypre_type -pc_hypre_boomeramg_strong_threshold'
  # petsc_options_value = 'hypre    boomeramg      0.01'

  # Use with solve_type = PJFNK in Executioner block
  # petsc_options_iname = '-pc_type -pc_factor_mat_solver_package'
  # petsc_options_value = 'lu superlu_dist'

  # Use with both solve_type = NEWTON and PJFNK in Executioner block
